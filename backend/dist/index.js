...
app.get('/api/rooms/:roomId', (req, res) => {
    try {
        const room = roomManager_1.RoomManager.getRoom(req.params.roomId);
        if (!room) {
            return res.status(404).json({ error: 'Room not found' });
        }
        res.json(roomManager_1.RoomManager.serializeRoom(room));
    }
    catch (error) {
...
io.on('connection', (socket) => {
    ...
    socket.on('create-room', (data) => {
        try {
            const validationError = roomManager_1.RoomManager.validateRoomSettings(data.settings);
            if (validationError) {
                socket.emit('error', validationError);
                return;
            }
            const room = roomManager_1.RoomManager.createRoom(currentPlayerId, data.username, data.avatar, data.settings);
            currentRoomId = room.id;
            socket.join(room.id);
            socket.emit('room-created', { roomId: room.id, room: roomManager_1.RoomManager.serializeRoom(room) });
    ...
    socket.on('join-room', (data) => {
        try {
            const { room, error } = roomManager_1.RoomManager.joinRoom(data.roomId, currentPlayerId, data.username, data.avatar);
            if (error) {
            ...
            socket.emit('room-joined', { room: roomManager_1.RoomManager.serializeRoom(room), playerId: currentPlayerId });
            socket.to(data.roomId).emit('room-updated', roomManager_1.RoomManager.serializeRoom(room));
    ...
    socket.on('start-game', () => {
        try {
            ...
            const gameState = gameLogic_1.GameLogic.startGame(room);
            io.to(currentRoomId).emit('game-started', gameState);
            io.to(currentRoomId).emit('room-updated', roomManager_1.RoomManager.serializeRoom(room));
    ...
    socket.on('player-action', (actionData) => {
        try {
            ...
            const { gameState, error } = gameLogic_1.GameLogic.processPlayerAction(room, action);
            if (error) {
                socket.emit('error', error);
                return;
            }
    ...
    socket.on('update-balance', (data) => {
        try {
            ...
            const success = gameLogic_1.GameLogic.updatePlayerBalance(room, data.playerId, data.newBalance);
            if (success) {
    ...
    socket.on('update-settings', (settings) => {
        try {
            ...
            gameLogic_1.GameLogic.updateRoomSettings(room, settings);
            io.to(currentRoomId).emit('room-updated', roomManager_1.RoomManager.serializeRoom(room));
    ...
    function handleLeaveRoom() {
        if (!currentRoomId)
            return;
        try {
            const { room, shouldDeleteRoom } = roomManager_1.RoomManager.leaveRoom(currentRoomId, currentPlayerId);
            if (shouldDeleteRoom) {
                console.log(`Room deleted: ${currentRoomId}`);
            }
            else if (room) {
                socket.to(currentRoomId).emit('player-left', currentPlayerId);
                socket.to(currentRoomId).emit('room-updated', roomManager_1.RoomManager.serializeRoom(room));
