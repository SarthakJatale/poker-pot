...
export class GameService {
    ...
    try {
      ...
      switch (action.action) {
        ...
        case 'raise':
          ...
          Logger.debug(`Player raised`, { playerId: action.playerId, amount: raiseAmount });
          break;
        ...
        case 'call':
          const maxCurrentBet = Math.max(...players.map(p => p.currentBet));
          const callAmount = maxCurrentBet - currentPlayer.currentBet;
          if (callAmount <= 0) {
            return { gameState: room.gameState, error: 'Nothing to call' };
          }
          if (currentPlayer.balance < callAmount) {
            return { gameState: room.gameState, error: 'Insufficient balance to call' };
          }
  ...
  private static advanceToNextPhase(room: Room): { gameState: GameState; error?: string } {
    try {
      ...
      Logger.debug(`Game phase advanced`, { 
        roomId: room.id, 
        phase: room.gameState.roundPhase, 
        cardsOnTable: room.gameState.cardsOnTable 
      });

      return { gameState: room.gameState };
    } catch (error) {
      Logger.error('Failed to advance game phase', error);
      return { gameState: room.gameState, error: 'Failed to advance game phase' };
    }
  }
    ...
    try {
      ...
      if (winnerId) {
        ...
        if (winner) {
          ...
          Logger.info(`Round ended with winner`, { 
            roomId: room.id, 
            winnerId, 
            potWon: potAmount 
          });
        ...
        Logger.info(`Round ended with pot split`, { 
          roomId: room.id, 
  ...
  static updateRoomSettings(room: Room, settings: Partial<RoomSettings>): void {
    if (room.gameState.isGameInProgress) {
      throw new ValidationError('Cannot update settings during an active game');
    }
